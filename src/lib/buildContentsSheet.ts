...
import { copyCellProperties, copyHeaderColumnGroup, copyWorksheet } from "./copy";
...
const ROWS_PER_COLUMN_GROUP = 2; // Maximum rows per column group
...
export function buildContentsSheet({
	...
	const totalEntries = data.entitiesArray.length;
	const columnGroupsNeeded = Math.ceil(totalEntries / ROWS_PER_COLUMN_GROUP);
	const distinctNamesArray = data.entitiesArray;
	...
	for (let groupIndex = 1; groupIndex < columnGroupsNeeded; groupIndex++) {
		...
		copyHeaderColumnGroup({
			templateSheet: contentsTemplate,
			targetSheet: contentsSheet,
			sourceStartCol,
			sourceEndCol,
			targetStartCol,
	...
	for (let groupIndex = 0; groupIndex < columnGroupsNeeded; groupIndex++) {
		...
		for (let i = 0; i < entriesInThisGroup; i++) {
			...
			const nameCell = contentsSheet.getCell(rowIndex, groupStartCol + NAME_COLUMN_OFFSET);
			...
			const linkCell = contentsSheet.getCell(rowIndex, groupStartCol + LINK_COLUMN_OFFSET);
			// Copy all properties from template cell first
			...
			const nextCell = contentsSheet.getCell(rowIndex, groupStartCol + NEXT_COLUMN_OFFSET);
